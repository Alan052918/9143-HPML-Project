...
class EstimatorSpecBuilder(object):
    ...
    * In predict mode:      it takes as input and outputs waveform. The whole
    ...
    def _build_optimizer(self):
        ...
        if name == self.ADAGRAD:
            return tf.compat.v1.train.AdagradOptimizer(rate)
        if name == self.RMSPROP:
            return tf.compat.v1.train.RMSPropOptimizer(rate)
        if name == self.NESTEROV:
            return tf.compat.v1.train.MomentumOptimizer(rate, momentum=0.9, use_nesterov=True)
        return tf.compat.v1.train.AdamOptimizer(rate)
